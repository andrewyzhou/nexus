    ...
    async def _run_batch(
        ...
        async with AsyncSession(impersonate="chrome") as session:
            try:
                ...
                # crumb endpoint rate-limited. Don't treat as terminal — the
                # outer loop will re-queue this batch after the normal run.
                print(f"  [batch auth failed] {e} — will retry at end", file=sys.stderr)
                return [], list(tickers), True
